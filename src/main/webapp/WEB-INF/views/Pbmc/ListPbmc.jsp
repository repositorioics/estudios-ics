        ...
        <div class="container-fluid">
            ...
            <div class="card-body bg-primary text-white mailbox-widget pb-0">
                <h2 class="text-white pb-3">
                    <i class="fa fa-flask text-white" aria-hidden="true"></i>
                    <spring:message code="Recepcion" />
                    <spring:message code="Pbmc" /></h2>
                <ul class="nav nav-tabs custom-tab border-bottom-0 mt-4" id="myTab" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" id="sent-tab" data-toggle="tab" aria-controls="sent" href="#sent" role="tab" aria-selected="false">
                            ...
                            <span class="d-none d-md-block"><spring:message code="Form" /> <spring:message code="Envío" /><spring:message code="PBMC" /></span>
                        </a>
                    </li>
                </ul>
            </div>
            ...
            <div>
                <a class="btn btn-info btn-lg" href="<spring:url value="/Pbmc/recepcion" htmlEscape="true "/>">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <spring:message code="add"/> <spring:message code="Pbmc"/>
                </a>
                <hr/>

                <spring:url value="/Pbmc/closeCase" var="closeUrl"/>
                ...
                <spring:url value="/Pbmc/sendAllPbmc" var="sendAllPbmcUrl"/>

                <div class="table-responsive">
                    <table id="lista_pbmc" class="table table-hover table-bordered">
                        <thead>
                        <tr>
                            ...
                            <th class="text-center" width="12%"><spring:message code="Tiene Rojo?" /></th>
                            <th class="text-center" width="16%"><spring:message code="actions" /></th>
                        </tr>
                        </thead>
                        <tbody>
                        <c:forEach items="${pbmc}" var="p">
                            <spring:url value="/Pbmc/editPbmc/{pbcm_id}" var="editUrl">
                                <spring:param name="pbcm_id" value="${p.pbcm_id}" />
                            </spring:url>
                            <tr>
                                <td><fmt:formatDate value="${p.fecha_pbmc}" pattern="dd/MM/yyyy" /></td>
                                <td><c:out value="${p.codigo_participante}" /></td>
                                <td><c:out value="${p.volumen}" /></td>
                                <td><c:out value="${p.estudios}" /></td>
                                <td>
                                    <c:choose>
                                        <c:when test="${p.enviado =='1'}">
                                ...
                                <td class="text-center">
                                    <c:choose>
                                        <c:when test="${p.pbmc_tiene_serologia =='0'}">
                                            <%--<c:out value="${p.pbmc_tiene_serologia}"></c:out>--%>
                                            <i class="fa fa-times text-danger" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="No"></i>
                                        </c:when>
                                        <c:otherwise>
                                            <%--<c:out value="${p.pbmc_tiene_serologia}"></c:out>--%>
                                ...
                                <td align="center">
                                    <a title="<spring:message code="edit" />" href="${fn:escapeXml(editUrl)}" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></a>
                                    <a title="<spring:message code="delete" />" data-toggle="modal" data-id="${p.pbcm_id}" class="btn btn-danger btn-sm salida"><i class="fa fa-trash text-white"></i></a>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
            ...
            <div class="tab-pane fade" id="sent" aria-labelledby="sent-tab" role="tabpanel">
                ...
                <!-- inicia form inline -->
                <div class="container">
                    <form name="envio-allpbmc-form" id="envio-allpbmc-form" autocomplete="off">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form action="#" autocomplete="off" id="close-form" name="close-form" class="form-horizontal">
                            <input type=hidden class="form-control" id="pbmc_id" name="pbmc_id"/>
...
<spring:url value="/resources/js/views/Pbmc/ListPbmc.js" var="PbmcJs" />
<script type="text/javascript" src="${PbmcJs}"></script>

<script>
    $(document).ready(function(){
        var misUrl ={
            "successLabel"        : "${successLabel}",
            "sendAllPbmcUrl"      : "${sendAllPbmcUrl}",
            "dataTablesLang"      : "${dataTablesLang}",
            "editUrl"             : "${editUrl}",
            "closeUrl"            : "${closeUrl}",
            "listSerologiaUrl"    : "${listSerologiaUrl}"
        }
        ListPbmc.init(misUrl);
        $("#desde").datepicker({
            format: "dd/mm/yyyy",
            todayBtn:true,
            todayHighlight: true,
        ...
        function PbmcPasive(){
            ...
            $.post( misUrl.closeUrl, form2.serialize(), function( data ){
                registro = JSON.parse(data);
                debugger;
                if (registro.pbcm_id ===undefined || registro.pbcm_id==null) {
                    swal({
                        title: "¡INFORMACIÓN!",
                        text: data.msj,
                        type: "error",
